 ...
 *
 * @param subscribeName
 * @returns {{lastFetchedItem: {id: string, pubDate: string}, allItems?: {id: string}[]}}
 */
...
export const fetchRSS = async (subscribeName, url, options = undefined) => {
    try {
        ...
        if (Cfg.Default.proxy) {
            ...
            logger.debug(`${url}: Using proxy: ${Cfg.Default.proxy}`)
        }
        ...
        if (raw.feed) {
            ...
            raw.rss.channel[0]?.item && raw.rss.channel[0].item.forEach(entry => {
                rawItems.push(entry)
            })
        }
        // const oneDayAgo = new Date();
        // oneDayAgo.setDate(oneDayAgo.getDate() - 1);
        let latestItem, allItems = []
